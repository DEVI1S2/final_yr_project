...
function Products() {
  ...
  const getAdminInfo = ()=> {
    ...
    Axios.get("http://localhost:3001/vendor/getshopadminfo/"+shopid)
    .then((response)=>{
  ...
  const getProducts = () =>{
    let shopid = localStorage.getItem('shop_ID');
    Axios.get("http://localhost:3001/vendor/getproducts/"+shopid)
    .then((response)=>{
  ...
  const deleteProd = (product)=> {
   ...
   if( window.confirm('Are you sure want to delete this item?')){
    Axios.delete("http://localhost:3001/vendor/deleteproduct/"+prodid)
    .then((response)=>{
