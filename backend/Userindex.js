...
function router(){
    ...
    userreg.post('/', async(req, res) =>{//on anything go to index.js
       ...
       console.log(req.body);
        ...
        db.query("INSERT INTO user(first_name,middle_name,last_name,home_addr,email,phno,password,recoveryque) VALUE (?,?,?,?,?,?,?,?)",
    ...
    db.query("SELECT * FROM user WHERE email = ? ",[emails],
    async(err,result)=>{
      console.log(result[0]);
      if (result.length > 0)
         {
    ...
    userreg.post('/forgotpassword',async(req, res) =>{
      ...
      db.query("SELECT * FROM user WHERE email = ? && recoveryque = ?",[email,recoveryque],
      async(err,result)=>{
